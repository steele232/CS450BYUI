...
oldTrainRows <- 1:nrow(lettersTrain)
cvRows <- sample(oldTrainRows, trunc(length(oldTrainRows) * 0.3))
lettersCV <- lettersTrain[cvRows,]
lettersTrain <- lettersTrain[-cvRows,]
...
for (g in params.to.try) {
  for (c in params.to.try) {
    ...
    prediction <- predict(model, lettersCV[,-1])
    
    # Produce a confusion matrix
    confusionMatrix <- table(pred = prediction, true = lettersCV$letter)
    
    # Calculate the accuracy, by checking the cases that the targets agreed
    agreement <- prediction == lettersCV$letter
